...
class PlacementChart extends HookWidget {
  const PlacementChart({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    useEffect(() {
      () async {
        try {
          final res = await ReportService.getPlacementCounts();
          data.value = res;
        } catch (e) {
          debugPrint("Placement Chart Error: $e");
        } finally {
          loading.value = false;
        }
      }();
    ...
    return Column(
      ...
      children: [
              ...
              AppButton(
                ...
                onPressed: () {
                  final rows = PdfExporter.mapToRowList(
                    data: data.value,
                    keys: ["placement", "total"],
                  );

                  PdfExporter.exportTable(
                    title: "Placement Report",
                    headers: ["Placement", "Total Count"],
                    rows: rows,
                  );
                },
              )
        ...
        if (selectedTab.value == "table")
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 10),
            child: _PlacementTable(data: data.value),
          ),
...
//
//=====================================================
// 🔹 PLACEMENT TABLE (Full width, no overflow)
//=====================================================
//
class _PlacementTable extends StatelessWidget {
  final List<Map<String, dynamic>> data;

  const _PlacementTable({required this.data});
          ...
          rows: data.asMap().entries.map((entry) {
            ...
            return DataRow(
              cells: [
                ...
                DataCell(Text(row["placement"] ?? "N/A")),
...
//
//=====================================================
// 🔹 Tab Button (modern, consistent style)
//=====================================================
//
