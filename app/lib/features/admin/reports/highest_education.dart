...
class HighestEducationChart extends HookWidget {
  const HighestEducationChart({super.key});

  @override
  Widget build(BuildContext context) {
    final loading = useState(true);
    final data = useState<List<Map<String, dynamic>>>([]);
    final selectedTab = useState("table"); // default view = table

    // Fetch API
    useEffect(() {
      () async {
        try {
          final res = await ReportService.getHighestEducationCounts();
          data.value = res;
        } catch (e) {
    ...
    return Column(
      ...
      children: [
              ...
              AppButton(
                ...
                onPressed: () {
                  final rows = PdfExporter.mapToRowList(
                    data: data.value,
                    keys: ["highest_education", "total"],
                  );

                  PdfExporter.exportTable(
                    title: "Highest Educational Attainment Report",
                    headers: ["Educational Level", "Total Count"],
                    rows: rows,
                  );
                },
              )
        ...
        if (selectedTab.value == "table")
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 10),
            child: _EducationTable(data: data.value),
          ),
                              ...
                              getTitlesWidget: (value, _) {
                                ...
                                return Padding(
                                  ...
                                  child: RotatedBox(
                                    quarterTurns: 1,
                                    child: Text(
                                      data.value[index]["highest_education"],
...
//
//=====================================================
// 🔹 EDUCATION TABLE (Full width, non-scrollable)
//=====================================================
//
class _EducationTable extends StatelessWidget {
  final List<Map<String, dynamic>> data;

  const _EducationTable({required this.data});

  @override
  Widget build(BuildContext context) {
    return Container(
      width: double.infinity, // full width area
      child: FittedBox(          // ✅ FIX: prevents overflow
        fit: BoxFit.scaleDown,   // shrink table if needed
            ...
            rows: data.asMap().entries.map((entry) {
              ...
              return DataRow(
                cells: [
                  ...
                  DataCell(Text(row["highest_education"] ?? "N/A")),
...
//
//=====================================================
// 🔹 TAB BUTTON WIDGET (Cleaner, reusable)
//=====================================================
//
