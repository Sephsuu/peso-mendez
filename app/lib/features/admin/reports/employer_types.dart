...
class EmployerTypeChart extends HookWidget {
  const EmployerTypeChart({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    // Fetch employer type counts
    useEffect(() {
      () async {
        try {
          final res = await ReportService.getEmployerTypesCounts();
          data.value = res;
        } catch (e) {
          debugPrint("Employer Type Chart Error: $e");
        } finally {
          loading.value = false;
        }
      }();
    ...
    if (data.value.isEmpty) {
      return const Center(child: Text("No employer type data available"));
    }
    ...
    return Column(
      ...
      children: [
              ...
              AppButton(
                ...
                onPressed: () {
                  final rows = PdfExporter.mapToRowList(
                    data: data.value,
                    keys: ["employer_type", "total"],
                  );

                  PdfExporter.exportTable(
                    title: "Employer Type Report",
                    headers: ["Employer Type", "Total Count"],
                    rows: rows,
                  );
                },
              )
        ...
        if (selectedTab.value == "table")
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 10),
            child: _EmployerTypeTable(data: data.value),
          ),
                              ...
                              getTitlesWidget: (value, _) {
                                ...
                                return Padding(
                                  ...
                                  child: RotatedBox(
                                    quarterTurns: 1,
                                    child: Text(
                                      data.value[index]["employer_type"],
...
//
//=====================================================
// 🔹 EMPLOYER TYPE TABLE (Scrollable)
//=====================================================
//
class _EmployerTypeTable extends StatelessWidget {
  final List<Map<String, dynamic>> data;

  const _EmployerTypeTable({required this.data});
          ...
          rows: data.asMap().entries.map((entry) {
            ...
            return DataRow(
              cells: [
                ...
                DataCell(Text(row["employer_type"] ?? "N/A")),
...
//
//=====================================================
// 🔹 Tab Button (modern PESO UI style)
//=====================================================
//
